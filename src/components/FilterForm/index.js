  ...
  render() {
    ...
    return(
      <form className="filter-events-form">
        ...
        <button type="button" className="filter-events-submit" onClick={() => {_filterByCategory(this.state.categoryId); this.setState({categoryId: null});}}><Link to={(this.state.categoryId === null) ? 'events' : `/events?categoryId=${this.state.categoryId}`}>Filter</Link></button>
      </form>
    );
  }
}
